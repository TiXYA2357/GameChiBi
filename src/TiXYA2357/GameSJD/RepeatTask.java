...
public class RepeatTask extends PluginTask<Main> {
    ...
    private static void endTo(){
        ...
        onesPlayer.forEach(map -> {if (map.get(lastWinner) != null)//获胜方处理
             map.get(lastWinner).forEach(string ->{ setPlayerInfoByUUID(string.split(":")[0],
            ...
            //战败方处理
            map.forEach((ones, str) -> {if (!ones.equals(lastWinner)) str.forEach(string -> {
